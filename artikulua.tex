...
As it can be seen, the best performing algorithms in this very basic baseline setting are Logistic Regression and SVM, which is not surprising given the ability of SVM to perform well with very few labeled data. Next, we decided to experiment with a less-sparse representation of the tweets by means of pre-trained word embeddings.
...
For the experiments, we used pre-trained clusters using the Elhuyar Web Corpus \cite{leturia2012evaluating} and from a 600M word corpus obtained from crawling local news sites. The number of clusters trained with each algorithm and data source was the following: 100-800 clusters using the Clark and Word2vec methods, and 1000, 2000 and 3200 classes with the Brown algorithm. The best combination of features was obtained by performing every possible permutation between them in a 5-fold cross validation setting using the gold standard training data. Following this methodology, the best configuration consisted of the following features (we did not use any local or lexicon-based features, just the clustering representations).

\begin{itemize}
\item Brown: Elhuyar Web Corpus 3200 classes.
\item Clark: Elhuyar Web Corpus 600 and Local News Corpus 300 clusters.
\item Word2vec: Elhuyar Web Corpus 300 and Local News Corpus 500 clusters.
\end{itemize}
...
\begin{table}[H]
  ...
  \end{tabular}
  \caption{ixa-pipe-doc results on training data via 5-fold cross-validation.}
  \label{tab:ixa-ml-garapen}
\end{table}


\subsubsection{Neural architecture}\label{sec:neural-architecture}
