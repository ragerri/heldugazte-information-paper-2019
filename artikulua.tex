...
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.49\linewidth}
    \includegraphics[width=\linewidth]{harremanak_heldu/c1for}
    \caption{Adults, most important subgroup (Nationalist left)}
  \end{subfigure}
  \begin{subfigure}[b]{0.49\linewidth}
    \includegraphics[width=\linewidth]{harremanak_gazte/c1inf}
    \caption{Youngs, most important subgroup (Sports)}
  \end{subfigure}
  \caption{Adult and Young Basque speakers' relations in Twitter}
  \label{fig:hd ha2}
\end{figure}
...
\todo{grafoa nola intepretatu, zer den ikusten dena}
Each node of the graph represents a user that interacts via retweet with other users, while each edge represent each of the retweets done. This way, the nodes that interact most with each other are grouped in subgroups, giving the opportunity to infer the different communities that conform the chosen object of study.
...
\todo{Aurreko tituluan "Relationships in the Adult Graph" daukagu, hemen Subgraph ordez Graph jarriko nuke ere}
...
We have shown how to develop different systems that allows us an approximation to the reality of certain communities of Twitter, in this case to the community of Basque speakers. The first step has allowed the collection of large amounts of information based on the target community. 
